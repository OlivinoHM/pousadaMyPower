...
If (CStr(Request("MM_update")) = "form1" And CStr(Request("MM_recordId")) <> "") Then
  ...
  MM_editRedirectUrl = "listar_lazer.asp"
...
atualiza.Source = "SELECT *  FROM lazer  WHERE id = " + Replace(atualiza__MMColParam, "'", "''") + ""
...
%>
<%
Dim cat2__MMColParam
cat2__MMColParam = "1"
If (Request.QueryString("id") <> "") Then 
  cat2__MMColParam = Request.QueryString("id")
End If
%>
<%
Dim cat2
...
cat2.Source = "SELECT *  FROM pacotes"
cat2.CursorType = 0
...
<!DOCTYPE html><html xmlns="http://www.w3.org/1999/xhtml" class="">
 ...
 <div> 
<br>
<strong class="text-primary fa-2x azul">  &Xi; Editar lazer</strong><br>
...
<form method="POST" action="<%=MM_editAction%>" name="form1">
  ...
  <table border="0" style="border-collapse: collapse" width="90%" cellpadding="0">
    ...
    <tr>
      ...
      <td width="455"><p>
        <select size="1" name="paclink">
      <%
While (NOT cat.EOF)
%>
      <option value="<%=(cat.Fields.Item("id").Value)%>" <%If (Not isNull((cat.Fields.Item("pactitulo").Value))) Then If (CStr(cat.Fields.Item("id").Value) = CStr((cat2.Fields.Item("paclink1").Value))) Then Response.Write("selected=""selected""") : Response.Write("")%>>(Pacote)&nbsp;-&nbsp;<%=(cat.Fields.Item("pactitulo").Value)%></option>
      <%
  cat.MoveNext()
Wend
